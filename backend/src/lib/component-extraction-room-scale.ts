/**
 * DUPLICATE of worker `design-engine/component-extraction/roomScaleForExtraction.ts` — keep in sync.
 * Used on GET /projects/:id/components to repair bare S/M/L and legacy rows.
 */
...
export interface ComponentExtractionRow {
  ...
  placement: string;
  wallLocation: string;
  suggestedBuyLinks: Array<{ label: string; url: string; note?: string }>;
  confidence: number;
}
...
// --- API read-path: repair stored JSON (bare S/M/L, snake_case fields) ---

const VALID_COMPONENT_CATEGORIES = new Set<string>([
  'Furniture',
  'Fixed Components',
  'Materials & Finishes',
  'Lighting',
...
export function normalizeExtractedComponentCategory(raw: unknown): ComponentCategory {
  const s = String(raw ?? '').trim();
  if (VALID_COMPONENT_CATEGORIES.has(s)) {
    return s as ComponentCategory;
  }
...
export function pickApproximateSizeField(row: Record<string, unknown>): string {
  const v =
    row.approximateSize ??
...
/** Recompute approximateSize when DB/S3 still has S/M/L (or missing digits). */
export function reApplyNumericApproximateSizesToStoredRows(
  rows: Record<string, unknown>[] | undefined,
  room: { type: string; geometry?: unknown; metadata?: unknown }
): Record<string, unknown>[] {
  if (!Array.isArray(rows)) return [];
  const metrics = extractRoomMetricsFromRoom(room, room.type);
  return rows.map((raw) => {
    const approximateSize = stripBareSizeLetterLabel(pickApproximateSizeField(raw).trim());
    const componentCategory = normalizeExtractedComponentCategory(raw.componentCategory);
    const cr: ComponentExtractionRow = {
      roomName: String(raw.roomName ?? ''),
      componentCategory,
      componentName: String(raw.componentName ?? ''),
      description: String(raw.description ?? ''),
      material: String(raw.material ?? ''),
      finishColor: String(raw.finishColor ?? ''),
      approximateSize,
      placement: String(raw.placement ?? ''),
      wallLocation: String(raw.wallLocation ?? ''),
      suggestedBuyLinks: Array.isArray(raw.suggestedBuyLinks)
        ? (raw.suggestedBuyLinks as ComponentExtractionRow['suggestedBuyLinks'])
        : [],
      confidence:
    ...
    const out = ensureNumericApproximateSize(cr, metrics);
    return { ...raw, ...out, componentCategory: out.componentCategory };
  });
}
