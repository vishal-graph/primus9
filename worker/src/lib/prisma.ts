 ...
 *
 * Set `WORKER_PRISMA_CONNECTION_LIMIT` (default 3) to cap pool size; use `1` if
 * your DB pool is very small (share the process with API + many services).
 */
...
function buildWorkerDatabaseUrl(): string {
  ...
  const limit = process.env.WORKER_PRISMA_CONNECTION_LIMIT ?? '3';
