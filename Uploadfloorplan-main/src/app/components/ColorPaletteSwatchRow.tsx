...
interface ColorPaletteSwatchRowProps {
  swatchColors: string[];
  selectedSwatchIndex: number; // Now always a number (1-6), never null
  onSwatchSelect: (index: number) => void; // Now always passes a number
  paletteKey: string; // Used to trigger animation reset
}

export function ColorPaletteSwatchRow({ 
  swatchColors, 
  selectedSwatchIndex, 
  onSwatchSelect,
  paletteKey 
}: ColorPaletteSwatchRowProps) {
  const [animationKey, setAnimationKey] = useState(0);

  // Reset animation when palette changes
  useEffect(() => {
